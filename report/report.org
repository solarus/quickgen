...
\algnewcommand\Error[1]{\textbf{error}\ #1}
...
** Matching expressions
   ...
   we may have the two values \(foo :: a \ri Int \ri b\) and \(bar ::
   Int \ri Int \ri Bool\). If we try to match the type of \(foo\) with
   \(bar\) then the substitution \(\{ a \mapsto Int, b \mapsto Bool\}
   \) will be returned. If no match is found a failure is returned
   instead.
   ...
   \Function{MatchAux}{$t_1, t_2$}
     \CaseOf{$t_2$}
       ...
       \Case{$C(y_1, \ldots, y_n)$}
         \If{$t_1$ is not the same type constructor as $t_2$,~~$t_1 \neq C(x_1, \ldots, x_n)$ \footnote{For some types $x_1 \ldots x_n$.}}
           \State No match found. \Return{Failure}
         \Else
   ...
   - Let $t_1 = [Int]$ and $t_2 = (a \ri b)\ri[a]\ri[b]$.
     ...
     $[Int]$ and $[b]$.
     - In \textsc{MatchAux} the second case will match with $C=[\,]$
       and $y_1=b$. Since $t_1$ is also of this form, $C=[\,]$ and
       $x_1=Int$, then \textsc{MatchAux}$(Int, ~b)$ will be called
       recursively.
