...
#+LATEX_HEADER: \usepackage{ifdraft, listings}
...
** a Do-it-yourself High-Assurance compiler
   ...
   \end{haskellcode}
   \caption{The fibonacci sequence as defined in Copilot.}
   \end{listing}
...
*** A Copilot generator in \quickgen
    ...
    and included instead. The complete API can be found in listing
    \ref{lst:copilot-api} below:
    ...
    lang =
      $(defineLanguage
          [| ( sing             :: a -> [a]
             ...
             , cycleBool        :: Stream Bool -> Stream Bool
             , appBool          :: Stream Bool -> Stream Bool -> Stream Bool
             , and              :: Stream Bool -> Stream Bool -> Stream Bool
             , or               :: Stream Bool -> Stream Bool -> Stream Bool

             , cycleWord64      :: Stream Word64 -> Stream Word64
             ...
             , appWord64        :: Stream Word64 -> Stream Word64 -> Stream Word64
    ...
    tactic as employed by the Copilot generator since the
    \inlhask{s\_i} expressions constructed by the latter generator can
    use any of the other values \inlhask{s\_j} in the final
    expression, including \inlhask{s\_i} itself. Since recursion in
    streams can still be achieved using the \inlhask{cycle*}
    ...
    genSpec :: StdGen -> CopilotSpec
    genSpec g = let (numStreams, g1)  = randomR (2,12) g
                    (numTriggers, g2) = randomR (1,6) g1
                    (l, streams, g3)  = genStreams numStreams g2
                    triggers          = genTriggers l numTriggers (1,5) g3
                in (streams, triggers)
    \end{haskellcode}
