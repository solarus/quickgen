...
#+LATEX_HEADER: \usepackage{lipsum, minted, xcolor, fancyhdr, xspace, algpseudocode, algorithm, mathtools}
...
\newcommand{\example}{\stepcounter{myexample}\textbf{Example} \thesection.\themyexample:\xspace}
...
* Algorithm
  ...
  \example Lets say an user wants to generate an expression of type
  $[Integer]$ using values and functions from the following API:
  ...
  anything. However, it does not make sense to choose a type for $a$
  right now since we may want to specialize it to some type discovered
  later. For now, we say that the type variable is undecided and we
  will introduce the special notation $?a$ to mean exactly this. In
  the end, the final specialized type for $map$ that we arrive at is
  $(?a \ri Int) \ri [?a] \ri [Int]$.
  ...
  respectively. To succeed, we need to generate these two new values
  in some order instead. For simplicty's sake, lets do it from left to
  right starting with $e_1$:
  ...
  - \emph{Generating $e_1$ with type $?a \ri Int$}:
    ...
    start generating $e_2$. In any case lets imagine that the choice
    for $\mathcal{C}$ was $n$ and continue generating $e_2$.

  - \emph{Generating $e_2$ with type $[?a]$}:
    ...
    goal. Lets assume the second one, $sing$, followed by $d$ is
    chosen, resulting in $e_2 = sign~d$. Now all sub goals have
    actually been solved and the term generation terminates.

  The final expression for this example would be $map~e_1~e_2 =
  map~(\lambda x \ri n)~(sing~d)$. Now lets consider if it was changed
  slighly, i.e. by assuming $(\lambda x \ri x)$ was generated instead
  of $(\lambda x \ri n)$ when generating $e_1$.
...
** Matching expressions
   ...
   \[ \textsc{Match}(Int,~a \rightarrow b) \Longrightarrow (?a \rightarrow Int) \]
   A normal type matching algorithm would fail on this particular
   ...
   however has lost its forall quantifier and been turned into the
   cryptic looking $?a$. The \textsc{Match} function actually only
   considers the return type of the second argument during matching.
   Therefore, in this example, the only type variable considered
   during matching was $b$. The rest of the forall quantified type
   variables, in this case $a$, will be turned from into undecided
   type variables, $?a$.

   The intuition is that the type $(a \rightarrow b)$ can be used to
   construct a value of type $Int$ if we give it another value of type
   $a$. Since $a$ is forall quantified then a value of any type can be
   given to a function of this type for it to produce an $Int$ value.
   ...
   - Let $\mathbb{U}$ be a set of guesses for undecided variables.
     ...
     $t_2$, and tries to return a specialized version of the type
     $t_2$, which can be used in the construction of $t_1$. In
     addition to returning the updated type, the set of guesses might
     be updated during matching. If no match could be found then a
   ...
   variables into $Undecided$ ones. The step that actually finds a
   substitution is found in \textsc{MatchAux} function:
