...
** Matching expressions
   ...
   \todo{Använder ny notation $\Longrightarrow$ (evaluerar till). Introducera för läsaren.}
   ...
   \inlhask{bar :: Int -> Int -> Bool}. If we try to match the type of
   \inlhask{foo} with \inlhask{bar} then the substitution \([ a
   \mapsto Int, b \mapsto Bool] \) will be returned. If no match is
   found an error might be returned instead.
   ...
   forall quantified type variables. Lets look at one example to see
   the difference.

   \[ \textsc{Match}(Int,~\forall~a~b.~a \rightarrow b) \Longrightarrow (?a \rightarrow Int) \]

   A normal type matching algorithm would fail on this particular
   input since a value type cannot be matched against a function.
   \textsc{Match} however returns the type $(?a \rightarrow Int)$.
   ...
   The intuition is that the type $(\forall~a~b.~a \rightarrow b)$ can
   be used to construct a value of type $Int$ if we give it another
   value of type $a$. Since $a$ is forall quantified then a value of
   any type can be given to a function of this type for it to produce
   an $Int$ value.

   With this small introduction it is time to look at the algorithm
   for the match function.

   Let $\mathbb{U}$ be a set of guesses for undecided variables. Then
   the \textsc{Match} function takes two types, $t_1$ and $t_2$, and
   returns a specialized version of the type $t_2$, which can be used
   in the construction of $t_1$, or returns an error if $t_2$ cannot
   be matched with $t_1$. \textsc{Match} may also introduce guesses in
   \mathbb{U} for undecided type variables found in $t_1$.

   \begin{algorithmic}[1]
   \Function{Match}{$t_1, t_2$}
     \State $s \gets$ \Call{Match'}{$t_1, t_2$}
   \EndFunction
   \end{algorithmic}
