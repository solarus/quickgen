...
** Generating expressions
   ...
   Let $uses$ be some natural number, $ctx$ a context and $gs$ a list
   ...
   \begin{algorithmic}[1]
   \Function{Generate'}{$t$}
     \If{$t$ is a function type $t_1, \ldots, t_m$}
       \State Generate unique variable names $x_1, \ldots, x_{m-1}$
       \State Add the constructors $(uses, (x_i, t_i)),~1 \leq i < m$ to $ctx$
       \State $v \gets$ \Call{Generate'}{$t_m, ctx$}
       \State Remove the $x_i,~1 \leq i < m$ constructors from $ctx$
       \If{$v$ is \texttt{Just} an expression}
         \State \textbf{return} $(\lambda x_1 \ldots x_{m-1} \rightarrow v)$
