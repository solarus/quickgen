...
** Matching expressions
   ...
   however has lost its forall quantifier and been turned into a
   undecided type variable as explained above. Further, the
   \textsc{Match} function only considers the return type of the
   second argument during matching. Therefore, in this example, the
   only type variable considered during matching was $b$. All of the
   remaining for all quantified type variables, in this case $a$, will
   be turned into undecided type variables, here $?a$.
