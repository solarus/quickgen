...
** Problem and goal
   ...
   The problem is that there currently is no automatic way to
   ...
   The goal is to implement a library that automatically constructs
   such generators. The functions and values produced from the
   generators should possibly have higher order and/or polymorphic
   types. The goal of the project is considered reached when the
   library has been used to successfully test a real world EDSL, such
   as \textsc{Feldspar} \cite{feldspar}.
...
** Generating expressions
   ...
   \Function{GenerateAux}{$t$}
     \If{$t$ is a function type $t_1 \rightarrow \ldots \rightarrow t_m$}
       ...
       \EndIf
     \Else~Retry up to 3 times \Comment{$t$ is here a value type}
...
*** Expressions
    ...
    defined for the \inlhask{Maybe} Haskell data type:
    ...
    It might be worth noting that this particular function,
    \inlhask{maybe}, is already present in the Haskell Prelude[fn:3].
    Defining it again is probably unnecessary. In any case, the last
    step is to simply add this function to the API.
...
*** Calculating a class environment
    ...
    - Loop until $ns$ is empty:
      ...
      4. Extend $ns$ by adding all type classes mentioned in any of
         the instances in $is$, but not yet mentioned in neither $ns$
         nor $cenv$.
    - Return $cenv$
...
** Feldspar
   ...
   found in \autoref{appendix:feldspar-gen}.
   ...
   Using the API found in \autoref{appendix:feldspar-gen}, no bugs
   ...
   found in \autoref{appendix:feldspar}. The exact code used to
   compile and evaluate the generated expressions using the GHC API
   can be found in \autoref{listing:feldspar-compile} in \autoref{appendix:feldspar-gen}
...
*** Entailment and undecided variables
    ...
    our current goal type is =?a= and that
    \inlhask{mempty :: Monoid m => m} is the chosen constructor. This
    would introduce a guess for =?a= as
    #+BEGIN_LATEX
    \texttt{\color{purple}Monoid \color{black} ?a \color{violet} => \color{black} ?a}.
    #+END_LATEX
    If the type for =?a= is never fully realized, this could actually
    result in a compile error. To see why consider the following
    expressions:
    ...
    =?a=. In the second one, the type would have been further
    specialized to $[\texttt{?a}]$ giving us an unambiguous instance
...
* Conclusions
  ...
  The goal of this project, as stated in \autoref{sec:goal}, is to
  implement a reusable library for automatically constructing
  generators for EDSL's by specifying an API. A library was
  implemented, and in \autoref{chap:examples}, we showed how to use
  the library to define generators for two different EDSL's, Copilot
  and Feldspar. In the case for Feldspar, the generator was simply
  defined by specifying an API together with a single call to a
  library function, very much in the spirit of the goal. However, to
  test the EDSL, another function had to be defined to compile the
  generated expressions into real values. Furthermore, due to type
  classes not being fully implemented, it did not generate expressions
  using the full range of types available in the language. Even so,
  the generated values were used to discover an artificially
  introduced bug in the language, leading us to conclude that the
  library can be used today for simple regression testing.

  For \quickgen to become fully usable, and to completely reach the
  goal, the two issues, type classes and the need for compiled
  expressions, has to be solved. In the case for type classes, some of
  the work, calculating class environments, is already done. This was
  discussed in \autoref{sec:calc-ce}. The actual entailment of type
  classes should be possible to implement using the discussion in
  \autoref{sec:fut-CE} as a base. To compile generated expressions, it
  might be possible to include a specialized wrapper for the GHC API
  in this library, making this process easy. In
  \autoref{sec:fut-compiling}, another tactic used by \citeauthor{mh},
  is also discussed.

  All in all, \quickgen is a big step forward in achieving the goal.
  Using the library and this thesis as a base, the goal can be
...
* Feldspar generator specification
  ...
  main = do
      [n] <- getArgs
      g   <- getStdGen
      let rs = P.take (P.read n) $ randoms g :: [Seed]
          vals :: [Q.Exp]
          vals = P.map fst . catMaybes . P.map gen $ rs

      let go = runGhc (Just libdir) $ do
              _ <- getSessionDynFlags >>= setSessionDynFlags
              addTarget =<< guessTarget "Language.hs" Nothing
              load LoadAllTargets

              setContext [ IIDecl . simpleImportDecl . mkModuleName $ "Prelude"
                         , IIDecl . simpleImportDecl . mkModuleName $ "Language"
                         ]

              cs <- mapM (compileExpr . show) vals
              P.length cs `P.seq` return (P.zip cs vals)

      res <- catch go $ \e -> P.error $ "Should not happen" P.++ show (e :: SomeException)

      forM_ res $ \(p, v) -> do
          let p' = unsafeCoerce p :: Data WordN
              doIt e prog = do
                  let r = e prog
                  r `P.seq` return (P.Right r)
              f e prog = catch (doIt e prog) $ \e -> return (P.Left (show (e :: SomeException)))
          r1 <- f eval  p'
          r2 <- f eval' p'
          if r1 P./= r2
              then putStrLn $ "Not equal for: " P.++ show v
              else return ()
  \end{haskellcode}
  \caption{Code used to compile expressions generated by \texttt{gen} above. The calls to the different evaluation functions can be found on lines 28 and 29.}.\label{listing:feldspar-compile}
  \end{listing}
...
[fn:2] For instance \inlhask{lookupSubst :: Nat -> Substitution -> Maybe ([Variable], SType)} and \\ \inlhask{(|->) :: Nat -> SType -> Substitution}.
...
[fn:3] It can be found here: http://hackage.haskell.org/package/base-4.7.0.0/docs/Prelude.html#v:maybe.
