...
*** Generating polymorphic expressions
    ...
    - Imagine that we want to generate an expression with goal type $a
      ...
      \autoref{sec:example-map1} above, i.e. by generating names for
      the variables and adding the arguments with their respective
      types to the API. The goal type at this point would be $[a]$ and
      ...
      undecided type $?b \mapsto Int$ in \mathbb{U}. Our expression at
      this point would be: \[ \lambda~a_1~a_2 \ri
      ...
      alone, it would be perfectly reasonable to choose $a_1$ as a
      constructor for this goal since $a_1$ is for all quantified and
      can be matched with anything. However, if we substitute $e_1$ by
      $a_1$ and try to type check the resulting expression in, for
      instance, =ghci= we get the following:

      \begin{alltt}
      \(\lambda\)> :t (\(\lambda\) a_1 a_2 -> map undefined (sing (succ_int a_1))) :: a -> a -> [a]
    ...
    The problem above is that the types of $a_1$ and $a_2$ should not
...
** Generating expressions
   ...
   - Let $\textsc{Uses}(t)$ denote the number of uses for a
     constructor of type $t$ \ref{sec:algo-termination}, $\Gamma$ a
     context (API) and $\mathbb{U}$ a set of guesses for undecided
...
*** Expressions
    ...
    library. This limitation and why this has little effect on the
    usability of the library is discussed more in section
    \ref{sec:fut-expressions}.
...
*** Calculating a class environment
    ...
    solve this is discussed further in section \ref{sec:fut-CE}.
