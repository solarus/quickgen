...
generate = go . reverse
  where
    ...
    go (match : args) = do
        addToContext args
        Just m <- randomMatching match
        case m of
            MatchPrim id    -> do
                Prim (_, e, _) <- getId id
                return e
            MatchLambda c -> return (VarE (mkName [c]))

data MatchData = MatchPrim Int | MatchLambda Char

randBool :: GenContext Bool
randBool = randR (0,1) >>= return . (==1)

randomMatching :: Type -> GenContext (Maybe MatchData)
